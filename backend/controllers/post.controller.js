...
exports.postSomething = catchAsyncerrors(async (req, res, next) => {
    ...
    const { url = '', caption = '', location = '', commentsEnabled = true, visibility = 'public' } = req.body;
    ...
    if (url) {
        try {
            const uploadData = await cloudinary.uploader.upload(url,
                { folder: 'Posts' }
            );
            documentData.media = [
                {
                ...
                $or: [
                    { "profileData.profileVisibility": "public" },
                    {
                        $and: [
                            ...
                            { "profileData.friends": mongoose.Types.ObjectId(userId) }
                        ]
                    },
